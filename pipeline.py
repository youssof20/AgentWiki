...
def _run_inference_impl(task: str, write_back: bool, timeout_seconds: int) -> dict[str, Any]:
    ...
    with trace_ctx:
        ...
        with _span_ctx(langfuse, "run_static", {"task_len": len(task)}):
            r1 = run_static(task)
        ...
        with _span_ctx(langfuse, "run_agentwiki", {"task_len": len(task)}):
        ...
        with _span_ctx(langfuse, "score_run1", {}):
        ...
        if langfuse:
            try:
                ...
                if obs is not None:
                    obs.update(metadata={"scores": {"static": s1, "agentwiki": s2}, "delta": delta, "task_preview": task[:100]})
            except Exception:
                pass
...
def _span_ctx(lf, name: str, metadata: dict):
    """Context manager: Langfuse span around the block; step runs inside the span."""
